...
const ingestModule = (function () {
    ...
    async function display_packages() {
        ...
        try {
            ...
            // let ingest_user = JSON.parse(window.sessionStorage.getItem('repo_user'));

            // const id = ingest_user.uid;
            // const name = ingest_user.name;
            ...
            for (let i = 0; i < records.data.length; i++) {
                ...
                if (ingest_in_progress === 0) {
                   ...
                   //  html += `<button type="button" class="btn btn-sm btn-default run-qa" onclick="ingestModule.start_ingest('${batch}', '${job_uuid}');"><i class="fa fa-cogs"></i> <span>Start</span></button>`;

                    html += `<button type="button" class="btn btn-sm btn-default run-qa" data-batch="${batch}" data-job_uuid="${job_uuid}"><i class="fa fa-cogs"></i> <span>Start</span></button>`;

                    // nginx_path + '/dashboard/ingest/status?batch=' + batch + '&api_key=' + api_key + '&id=' + id + '&name=' + name + '&job_uuid=' + job_uuid + '"
            ...
            if (data.length > 0) {
                ...
                for (let i = 0; i < data.length; i++) {
                    if (data[i].error !== null && data[i].is_complete === 0) {
                        ...
                        message = '<div class="alert alert-info"><strong><i class="fa fa-info-circle"></i>&nbsp; An ingest is in progress.</strong><button id="stop-ingest" class="btn btn-default" type="button"></button></div>';
                        document.querySelector('#stop-ingest').addEventListener('click', () => {
    ...
    obj.init = async function () {
        ...
        document.querySelectorAll('.run-qa').forEach(btn => {
            btn.addEventListener('click', function(event) {
                const button = event.currentTarget;
                ingestModule.start_ingest(button.dataset.batch, button.dataset.job_uuid);
            });
        });
    };
